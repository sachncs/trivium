...
from trivium.pipelines import vector as pipelines_vector  # noqa: F401
from trivium.pipelines.base import PipelineInput
from trivium.pipelines.registry import PipelineRegistry, get_pipeline
from trivium.reranking.registry import RerankerRegistry, get_reranker
...
def preflight_check(config) -> bool:
    ...
    if abs(observed - target) > tol:
        print(
            f"  WARN: |delta|={abs(observed - target):.4f} > {tol}. "
            "Implementation drift from Anserini.",
            flush=True,
        )
        return True
...
def _instantiate_reranker(slug: str) -> object | None:
    """Construct a reranker by slug; return None if instantiation fails.

    Errors are logged so the rest of the benchmark can proceed.
    """
    try:
        return get_reranker(slug)
    except KeyError as exc:
        print(f"  reranker {slug!r} not registered: {exc}", flush=True)
        return None
    except Exception as exc:  # noqa: BLE001 - surface any model-load failure
        print(f"  reranker {slug!r} failed to instantiate: {exc}", flush=True)
        return None
...
def main() -> None:
    ...
    p.add_argument(
        ...
        help="Comma-separated subset of pipeline modes",
    )
    p.add_argument(
        "--scales",
        default=None,
        help="Comma-separated subset of scales, e.g. '5000,100000'",
    )
    p.add_argument(
        "--encoders",
        default=None,
        help="Comma-separated subset of encoder slugs (default: all configured encoders)",
    )
    p.add_argument(
        "--rerankers",
        default=None,
        help="Comma-separated subset of reranker slugs (default: 'encoder')",
    )
    ...
    selected_encoders = (
        ...
        if args.encoders
        else [e.slug for e in config.encoders]
    )
    selected_rerankers = (
        [r.strip() for r in args.rerankers.split(",") if r.strip()]
        if args.rerankers
        else ["encoder"]
    )
    ...
    print(f"  rerankers:{selected_rerankers}", flush=True)
    ...
    for slug in selected_encoders:
        if slug not in enc_by_slug:
            print(f"  encoder {slug!r} not configured; skipping", flush=True)
            continue
        ...
        try:
            ...
            print(f"  encoder {slug!r} not registered: {exc}", flush=True)
            continue
        try:
            ...
            print(f"  encoder {slug!r} failed: {exc}", flush=True)
            continue

    rerankers_by_slug: dict[str, object] = {}
    for slug in selected_rerankers:
        rr = _instantiate_reranker(slug)
        if rr is not None:
            rerankers_by_slug[slug] = rr
            print(f"  loaded reranker {slug}", flush=True)
    ...
    for scale in config.scales:
        ...
        for mode_name in selected_modes:
            ...
            # For hybrid_rerank, instantiate a reranker per encoder so each
            # (mode, encoder, scale) row carries its own reranker slug.
            rerankers_for_mode: list[tuple[str, object]] = list(rerankers_by_slug.items())
            if mode_name == "hybrid_rerank" and not rerankers_for_mode:
                print("  hybrid_rerank requires --rerankers; skipping mode", flush=True)
                continue
            ...
            for enc_slug in selected_encoders:
                ...
                if qv is None:
                    continue
                if rerankers_for_mode:
                    for rr_slug, rr in rerankers_for_mode:
                        inp = PipelineInput(
                            documents=documents,
                            queries=queries,
                            qrels=qrels,
    ...
    CsvResultWriter.write(all_rows, args.output)
